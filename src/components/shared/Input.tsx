...
const Input = (props: InputProps) => {
	...
	if (props.as === "input" && props.type === "checkbox") {
		return (
			<label htmlFor={props.name} className="flex items-center gap-1 text-sm">
				<input
					type="checkbox"
					className="peer sr-only rounded accent-dark"
					{...props}
				/>
				<div className="relative h-4 w-4 cursor-pointer rounded border-2 border-dark transition-opacity after:absolute after:left-1/2 after:top-1/2 after:h-2 after:w-2 after:-translate-x-1/2 after:-translate-y-1/2 after:rounded-sm after:bg-dark after:opacity-0 peer-checked:after:opacity-100 peer-focus-visible:outline peer-focus-visible:outline-2 peer-focus-visible:outline-offset-2 shadow-protrusion-xs" />
